        ...
        for row in rows:
            ...
            try:
                ...
                if cpu_anomaly or mem_anomaly:
                    parts = []
                    if cpu_anomaly:
                        parts.append(f"CPU spike {cpu:.1f}% (threshold {cpu_state.threshold:.1f}%, Ïƒ={cpu_std:.2f})")
                    if mem_anomaly:
                        parts.append(f"MEM spike {mem:.1f}% (threshold {mem_state.threshold:.1f}%, Ïƒ={mem_std:.2f})")
